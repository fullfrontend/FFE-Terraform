...
Applications  
- WordPress : MariaDB, PVC wp-content, S3 optionnel, ingress cert-manager (prod), FQDN `<root_domain>`. Le staging Granges du Tilleul réutilise strictement le même module et la même configuration que le WordPress `fullfrontend.be`, avec `APP_ENV=dev`; seuls son namespace, son PVC, son compte/base MariaDB, son certificat et son Schedule Velero sont isolés.
