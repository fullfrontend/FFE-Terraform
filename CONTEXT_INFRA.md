...
- Règles app :  
  - WordPress : MariaDB + PVC wp-content, plugin S3 optionnel, ingress cert-manager en prod. Le staging Granges du Tilleul reprend la pile PHP-FPM 8.3 + Caddy du compose avec `APP_ENV=dev`; il est isolé dans son propre namespace, avec base/utilisateur MariaDB, import SQL initial, PVC uploads, certificat et sauvegarde Velero dédiés.
