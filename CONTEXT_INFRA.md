...
- Règles app :  
  - WordPress : MariaDB + PVC wp-content, plugin S3 optionnel, ingress cert-manager en prod. Le staging Granges du Tilleul réutilise le même module et la même configuration que `fullfrontend.be`, avec `APP_ENV=dev`; il est isolé dans son propre namespace, avec base/utilisateur MariaDB, PVC, certificat et sauvegarde Velero dédiés.
